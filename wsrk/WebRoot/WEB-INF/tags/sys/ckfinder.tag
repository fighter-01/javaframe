...
<%@ attribute name="type" type="java.lang.String" required="true" description="files、images、flash、thumb、khfiles、oaNotifyFiles"%>
...
<ol id="${input}Preview"></ol><c:if test="${!readonly}"><a href="javascript:" onclick="${input}FinderOpen();" class="btn">${selectMultiple?'添加':'选择'}</a><c:if test="${type != 'khfiles' }">&nbsp;<a href="javascript:" onclick="${input}DelAll();" class="btn">清除</a></c:if></c:if>
	...
	function ${input}FinderOpen(){//<c:if test="${type eq 'thumb'}"><c:set var="ctype" value="images"/></c:if><c:if test="${type ne 'thumb'}"><c:set var="ctype" value="${type}"/></c:if>
		var date = new Date(), year = date.getFullYear(), month = (date.getMonth()+1)>9?date.getMonth()+1:"0"+(date.getMonth()+1);
			...
			if(type_l == "khfiles"){
				...
				var url = "${ctxStatic}/ckfinder/ckfinder.html?type=${ctype}&start=${ctype}:/"+"&id=${uploadPath}";
		   
			}else if(type_l == "oaNotifyFiles"){
	...
	function ${input}Del(obj){
		...
		if( "${ctype}" == "khfiles"){
			if(confirm("确定要删除该文件吗？")){
				$.ajax({  
					type:'post',
					url:'${ctx}/rmyhform/khformview/deletefile',
					async:true,
					data: {"url":url},  
