...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/accounts/tbWorkInspect/list?flag=1">行领导对分管部门党风廉政建设工作检查记录列表</a></li>
		<shiro:hasPermission name="accounts:tbWorkInspect:edit"><li><a href="${ctx}/accounts/tbWorkInspect/form">行领导对分管部门党风廉政建设工作检查记录添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="tbWorkInspect" action="${ctx}/accounts/tbWorkInspect/list?flag=1" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="tbWorkInspect">
			<tr>
				...
				<td>
				<c:choose>
					    <c:when test="${tbWorkInspect.status == '5'}">
					    	<span style="color: red;">审核未通过</span>
					    	&nbsp;&nbsp;<a href="#" style="color: red;" onclick='remarkView("${tbWorkInspect.remarks}")'>查看原因</a>
					    </c:when>
					    <c:when test="${tbWorkInspect.status == '6'}">
					    部门领导审核中
					    </c:when>
					    <c:otherwise>
				...
				<!-- 未提交审核或者提交被退回后可以进行的操作 -->
					<c:if test="${tbWorkInspect.status == '1' || tbWorkInspect.status == '5' }">
						<a href="${ctx}/accounts/tbWorkInspect/form?id=${tbWorkInspect.id}" ><span style="color: red;">提交审核</span></a>
						<a href="${ctx}/accounts/tbWorkInspect/form?id=${tbWorkInspect.id}">修改</a>
						<a href="${ctx}/accounts/tbWorkInspect/delete?id=${tbWorkInspect.id}" onclick="return confirmx('确认要删除该部门落实党风廉政建设责任制情况半年自查表吗？', this.href)">删除</a>
					</c:if>
					<!-- 提交后或者审核完后可以进行的操作 -->
					<c:if test="${tbWorkInspect.status == '2' || tbWorkInspect.status == '3' || tbWorkInspect.status == '4' || tbWorkInspect.status == '6' }">
